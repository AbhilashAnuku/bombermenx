# bomb-core: a walk through the module
...
I keep saying this in stand-ups: bomb-core is the only piece of BomberMen-X that the server, the headless tests, and the desktop client all agree on. If a rule is not in bomb-core, it does not exist. The renderer can draw a particle wherever it likes, but the truth of the match -- who is alive on tick 1734, who owns the King node, which tile just turned into a SOLID because Sudden Death is closing in -- lives here, in plain Java, with no Swing, no Netty, no Gradle plugin magic. This document walks the module package by package the way I would walk a new teammate through it on a whiteboard.
...
- `com.bombermenx.core.world` -- `Arena`, `TileType`, `PowerUpType`, `ArenaTheme`.
- `com.bombermenx.core.entity` -- `Player`, `Bomb`, `Explosion`, `PowerUpItem`.
...
`Arena.generate(int width, int height, long seed, ArenaTheme theme)` is the only construction path. It is fully deterministic: same `(width, height, seed, theme)` produces byte-identical layouts. The algorithm:
...
4. `density = GameConfig.DEFAULT_DESTRUCTIBLE_DENSITY * theme.destructibleDensityMul`.
...
public enum ArenaTheme {
    CLASSIC(1.00f, 1.00f, 1.00f),
    INFERNO(0.85f, 0.85f, 1.15f),
    CRYO   (1.20f, 1.15f, 0.90f),
    REACTOR(1.00f, 0.65f, 1.00f);

    public final float destructibleDensityMul, fuseMul, powerMul;
    ArenaTheme(float d, float f, float p) {
        this.destructibleDensityMul = d; this.fuseMul = f; this.powerMul = p;
    }
}
```

INFERNO trades wall count for stronger bombs. CRYO is the opposite -- more walls, longer fuses, weaker rays. REACTOR is the showstopper: 65 % fuse means ~1.6 s instead of 2.5 s, which is why I tell SK to lock that theme behind the "Veteran" lobby filter.

`TileType` has four values: `FLOOR`, `DESTRUCTIBLE`, `SOLID`, `SUDDEN_DEATH_BLOCK`. `blocksExplosion()` returns `true` for everything except `FLOOR`. Explosion rays use that one method; they do not branch per type.

`PowerUpType` is the canonical list: `BOMB_UP`, `FIRE_UP`, `SPEED_UP`, `SHIELD`, `KICK`, `PIERCE`, `REMOTE`, `CORE_TOKEN`. `CORE_TOKEN` is what feeds the super-ability economy and is intentionally rolled separately (see "two-roll pickup drop" in the simulation doc).

## Entities

`Player` carries: `playerId`, `team`, `TilePos pos`, `Direction facing`, `boolean alive`, `int maxBombs`, `int activeBombs`, `int power`, `int speedTier`, `boolean hasShield`, `boolean hasKick`, `boolean hasPierce`, `boolean hasRemote`, `int tokens`, `int controlPoints`, `int score`, `int stepCooldownTicks`, `int nukeCooldownTicks`, `int dashCooldownTicks`. `applyPowerUp(PowerUpType)` is a `switch` -- BOMB_UP bumps `maxBombs`, FIRE_UP bumps `power`, SPEED_UP decrements the step cooldown floor, etc. No event bus, no listeners; the simulation writes the field, the snapshotter reads it on the same tick.

`Bomb` holds `ownerId`, `int x`, `int y`, `int fuseTicks`, `int power`, `boolean detonated`, `boolean pierce`. The fuse is set at place-time from `theme.fuseMul * GameConfig.DEFAULT_BOMB_FUSE_TICKS` and stored as an int -- no floats inside the tick loop.
...
public final class PlayerInput {
    ...
    public boolean triggerNuke;   // one-shot, server clears after consuming
    public boolean triggerDash;   // one-shot, server clears after consuming
}
```

`move` is the desired step direction. `placeBomb` is also one-shot in practice -- the server checks it on the tick the input arrived and does not repeat-place if the same input is replayed. Abilities are explicit so we can rate-limit them cheaply.
...
6. **advanceSuddenDeath** -- only in `SUDDEN_DEATH` mode and only when `currentTick % 30 == 0`. Take the next spiral cell, set it to `SUDDEN_DEATH_BLOCK`, kill any player standing there.
...
`Envelope` is the wrapper for every wire message: `MessageType type`, `long seq`, `long t` (server tick or wall-clock ms, depending on type), `Object payload`. The codec resolves the payload class from the `type`.
...
`WireCodec` is the Jackson layer:
...
If a rule cannot be expressed by editing one of those seven tick steps, it does not belong in the simulation -- it belongs in the renderer or in a lobby toggle. That single discipline is what keeps bomb-core small enough to read in one sitting.
